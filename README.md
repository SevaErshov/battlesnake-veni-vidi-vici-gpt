# Battlesnake HardML Ensemble

Этот вариант зафиксирован под runtime-модель **`ensemble`**.
Его удобно залить в отдельную ветку `feature/hardml-ensemble` и деплоить как отдельную змейку для сравнения в Battlesnake.
...
По умолчанию в этом варианте стоит `ensemble`, потому что на синтетическом holdout он дал лучший top-1 выбор хода среди легких моделей.
...
5. Если вообще всё плохо — возвращает `up`, чтобы сервер ответил валидным JSON.
